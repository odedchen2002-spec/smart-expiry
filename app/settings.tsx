...
import { getPendingInvitations } from '@/lib/supabase/queries/collaborations';
import { getRtlContainerStyles, getRtlTextStyles } from '@/lib/utils/rtlStyles';
import { LinearGradient } from 'expo-linear-gradient';
import { useFocusEffect, useRouter } from 'expo-router';
import React, { useCallback, useState } from 'react';
import { Alert, ActivityIndicator, Platform, ScrollView, StyleSheet, TouchableOpacity, View } from 'react-native';
...
export default function SettingsScreen() {
  ...
  // Quick determination: Only show loading spinner if we don't have user info yet
  // If user exists, we can show the screen immediately (assume owner until proven otherwise)
  // The worst case is a brief flash if it turns out to be a collaborator, but the 
  // conditional rendering below already handles that with the collaboration block at the top
  const showLoadingSpinner = ownerLoading && !user?.id;

  // Fetch pending invitations count when screen is focused
  ...
  // If trial is active, show as 'free' (trial). Otherwise, use the actual plan ('pro', 'pro_plus', or 'free')
  const isTrial = subscription?.isTrialActive || false;
  const actualPlan = subscription?.plan || 'free';
  const subscriptionTier: 'free' | 'pro' | 'pro_plus' = isTrial
    ? 'free'
    : (actualPlan === 'pro_plus' ? 'pro_plus' : (actualPlan === 'pro' ? 'pro' : 'free'));
  ...
  const confirmLogout = async () => {
    ...
    console.log('[Settings] Logout: Starting sign out process');
    
    try {
      // CRITICAL: Sign out FIRST, then clear cache, then navigate
      // This ensures session is cleared before navigation
      await signOut();
      console.log('[Settings] Logout: Sign out completed');
      
      // Clear TanStack Query cache to remove all cached data
      queryClient.clear();
      console.log('[Settings] Logout: Query cache cleared');
      
      // Small delay to let auth state settle and prevent race conditions
      await new Promise(resolve => setTimeout(resolve, 150));
      
      // Navigate to login after session and cache are cleared
      console.log('[Settings] Logout: Navigating to login');
      router.replace('/(auth)/login');
    } catch (error) {
      console.error('[Settings] Error during logout:', error);
      // Even if logout fails, clear cache and navigate to login screen
      queryClient.clear();
      router.replace('/(auth)/login');
    }
  };
  ...
  const getSubscriptionBadgeColor = (): string => {
    ...
    if (subscriptionTier === 'pro_plus') return '#9C27B0'; // Purple for Pro+
  ...
  const getSubscriptionDescription = (): string => {
    ...
    if (subscriptionTier === 'pro_plus') {
      return t('settings.subscriptionDescription.proPlus');
    }
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: THEME_COLORS.surfaceVariant }]} edges={[]}>
      ...
      {/* Show loading indicator while determining owner status (only for collaborators) */}
      {showLoadingSpinner ? (
        ...
        >
          {/* 1. Collaborator Info (FIRST - if not owner) */}
          {/* Quick check: if activeOwnerId !== user.id, we know immediately it's a collaborator */}
          {activeOwnerId && user?.id && activeOwnerId !== user.id && ownerProfile && (
          <Card style={styles.sectionCard} mode="outlined">
            <View style={styles.cardContentWrapper}>
              <Card.Content style={styles.cardContent}>
                <Text variant="titleMedium" style={[styles.sectionTitle, rtlText]}>
                  {t('settings.collaboration.title') || 'שיתוף פעולה'}
                </Text>
                ...
                <Button
                  ...
                  onPress={async () => {
                    ...
                    if (result.success) {
                      // Show message and reload app
                      Alert.alert(
                        t('common.success') || 'הצלחה',
                        t('settings.collaboration.leftSuccess') || 'עזבת את השיתוף בהצלחה',
                        [{
                      ...
                      Alert.alert(
                        ...
                        t('settings.collaboration.leftError') || 'לא ניתן לעזוב את השיתוף. נסה שוב מאוחר יותר.'
                      );
                    }
                  }}
                  ...
                  style={{ marginTop: 8 }}
                >
                  {t('settings.collaboration.leave') || 'עזוב שיתוף'}
                </Button>
              </Card.Content>
            </View>
          </Card>
        ...
        <Card style={styles.sectionCard} mode="outlined">
          <View style={styles.cardContentWrapper}>
            <Card.Content style={styles.accountCardContent}>
              ...
              >
                <View style={styles.settingContent}>
                  <View style={[styles.settingTitleRow, isRTLContext && styles.settingTitleRowRTL]}>
                    <Text variant="bodyLarge" style={[rtlText, styles.settingTitle]} numberOfLines={1}>
                      {t('settings.sharedWithMe') || 'שיתופים אליי'}
                    </Text>
                    {pendingInvitationsCount > 0 && (
                      <Badge size={20} style={styles.invitationBadge}>
        ...
        {/* 3. Subscription Section (only for owners) */}
        {isOwner && (
          <Card style={styles.sectionCard} mode="outlined">
            <View style={styles.cardContentWrapper}>
              <Card.Content style={styles.cardContent}>
                ...
                >
                  <View style={styles.settingContent}>
                    ...
                    <Text variant="bodySmall" style={[rtlText, styles.settingDescription]} numberOfLines={2}>
                      {loadingSubscription ? t('settings.loading') : getSubscriptionDescription()}
                    </Text>
                  </View>
                ...
                <View style={[styles.settingContent, rtlContainer]}>
                  <View style={rtlContainer}>
                    ...
                    <Text variant="bodySmall" style={[rtlText, styles.settingDescription]}>
                      {locale === 'en' ? 'Version 1.0.0' : 'גרסה 1.0.0'}
                    </Text>
                  </View>
                </View>
                <List.Icon
