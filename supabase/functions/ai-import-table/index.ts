 ...
 * Returns structured JSON with product names, expiry dates, and barcodes.
 */
...
const SYSTEM_PROMPT = `You are a data extraction assistant. Your task is to extract product information from images of tables, spreadsheets, or product lists.
...
/**
 * Parse AI response content and normalize items
 * Handles markdown code blocks and normalizes dates
 */
function parseAiItemsFromContent(aiContent: string): any[] {
  // Parse AI response (may contain markdown code blocks)
...
serve(async (req) => {
  ...
  try {
    ...
    const { imageBase64, ownerId } = requestBody;
    ...
    console.log('[AI Provider] Using OpenAI GPT-4o for table import');

    // Use OpenAI GPT-4o with vision API
        ...
        messages: [
          {
          ...
          },
            {
              ...
              content: [
                {
                  type: 'text',
                  text: 'This image contains a table or list of products with expiry dates. Extract all products and return ONLY valid JSON in the format: {"items": [{"name": "...", "expiryDate": "YYYY-MM-DD", "barcode": "..."}]}. If no products are found, return {"items": []}. Do not refuse or explain - only return JSON.',
                },
                {
                  type: 'image_url',
                  image_url: {
    ...
    if (!aiContent) {
      ...
      try {
        normalizedItems = parseAiItemsFromContent(aiContent);
        console.log('[OpenAI API] Successfully parsed', normalizedItems.length, 'items');
        ...
        return new Response(
          JSON.stringify({ 
            ...
            details: aiContent.substring(0, 200) // First 200 chars for debugging
          }),
          { 
            status: 500, 
            headers: { 
    ...
    console.log('[ai-import-table] Successfully processed', normalizedItems.length, 'items');
