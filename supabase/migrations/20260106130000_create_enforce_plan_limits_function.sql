-- Create a function to enforce plan limits (bypasses RLS)
-- This function runs with SECURITY DEFINER which allows it to bypass RLS
...
DECLARE
  ...
  v_keep_ids UUID[];
BEGIN
  -- Disable RLS for this function to allow updates
  -- This is safe because the function already validates ownership via p_owner_id
...
-- Add comment
COMMENT ON FUNCTION enforce_plan_limits IS 'Enforces subscription plan limits by locking items beyond the tier limit. Runs with elevated privileges to bypass RLS.';
